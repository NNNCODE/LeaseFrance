...
export function getDepositStatusMeta(status: DepositStatus) {
  switch (status) {
    case 'awaiting':
      return { label: 'A encaisser', variant: 'warning' as const }
    case 'held':
      return { label: 'Detenu', variant: 'default' as const }
    case 'returned':
      return { label: 'Restitue', variant: 'success' as const }
    case 'partial':
      return { label: 'Restitue partiellement', variant: 'warning' as const }
    case 'retained':
      return { label: 'Retenu', variant: 'danger' as const }
    default:
      return { label: 'Aucun depot', variant: 'muted' as const }
  }
}
